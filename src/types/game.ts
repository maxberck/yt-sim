...
export interface VideoProject{id:string;title:string;category:VideoCategory;format:VideoFormat;language:Language;isFeat:boolean;isSeries:boolean;seriesId?:string;episodeNumber?:number;totalHours:number;hoursDone:number;quality:number;hype:number;tasks:ProductionTask[];imageStock:string;thumbnail:string;published:boolean;totalTurnsRequired:number;turnsSpent:number;weeksDone:number;totalWeeks:number;isReadyToPublish:boolean}
export interface PublishedVideo{id:string;projectId:string;title:string;category:VideoCategory;format:VideoFormat;thumbnail:string;views:number;subscribersGained:number;revenueGenerated:number;publishedAt:string;week:number;seriesId?:string;episodeNumber?:number;lastSimulatedHour:number;velocity:number}
...
export type GameAction={type:'REST'}|{type:'SLEEP'}|{type:'WORK';hours:number}|{type:'CREATE_PROJECT';title:string;category:VideoCategory;format:VideoFormat;language:Language;imageStock:string;isSeries?:boolean;seriesName?:string}|{type:'PUBLISH_VIDEO';projectId:string}|{type:'TOGGLE_FACE'}|{type:'WORK_PROJECT';projectId:string;taskId:string}|{type:'CREATE_PLAYLIST';title:string;description:string}|{type:'ADD_TO_PLAYLIST';playlistId:string;videoId:string}|{type:'ACCEPT_OFFER';offerId:string};
